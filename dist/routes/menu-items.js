...
const menuItemsPlugin = async (fastify) => {
    ...
    fastify.delete('/:clientId/:id', async (req, reply) => {
        const { clientId, id } = req.params;
        try {
            const { rowCount } = await fastify.pg.query('DELETE FROM menu_items WHERE id = $1 AND client_id = $2', [id, clientId]);
            if (!rowCount) {
                return reply.code(404).send({ error: 'Menu item not found' });
            }
...
exports.default = menuItemsPlugin;
